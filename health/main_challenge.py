...
from params2 import *
from ml_logic.clean_data import clean_data, get_data, load_data_to_bq
...
def preprocess(df: pd.DataFrame) -> None:
    ...
    query = f"""
        ...
        FROM `{GCP_PROJECT}`.{BQ_DATASET}.csv
        """

    # Retrieve data using `get_data_with_cache`
    data_query_cache_path = Path(LOCAL_DATA_PATH).joinpath("raw", f"{BQ_DATASET}.csv")
    data_query = get_data(query=query,
...
#     query = f"""
#         SELECT * EXCEPT(_0)
#         FROM `{GCP_PROJECT}`.{BQ_DATASET}.processed
#         WHERE _0 BETWEEN '{min_date}' AND '{max_date}'
#         ORDER BY _0 ASC
#     """

#     data_processed_cache_path = Path(LOCAL_DATA_PATH).joinpath("processed", f"processed_data.csv")
...
#     query = f"""
#         SELECT * EXCEPT(_0)
#         FROM `{GCP_PROJECT}`.{BQ_DATASET}.processed_data
#         WHERE _0 BETWEEN '{min_date}' AND '{max_date}'
#     """

#     data_processed_cache_path = Path(f"{LOCAL_DATA_PATH}/processed/processed_data.csv")
